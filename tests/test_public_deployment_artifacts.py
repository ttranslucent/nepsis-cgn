...
def test_live_operator_web_surface_uses_packet_routes_only() -> None:
    operator_proxy_dir = ROOT / "nepsis-web" / "src" / "app" / "api" / "engine" / "operator"
    assert not operator_proxy_dir.exists()

    web_sources = [
        ROOT / "nepsis-web" / "src" / "lib" / "engineClient.ts",
        ROOT / "nepsis-web" / "src" / "lib" / "useEngineSession.ts",
        ROOT / "nepsis-web" / "src" / "app" / "engine" / "page.tsx",
    ]
    text = "\n".join(path.read_text(encoding="utf-8") for path in web_sources)

    assert "/operator-packet/start" in text
    for legacy_path in [
        "/operator/session",
        "/operator/frame",
        "/operator/report",
    ...
    assert "operatorRuntime" not in text
